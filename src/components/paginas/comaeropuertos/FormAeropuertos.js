...
class FormAeropuertos extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  saveData(event) {
    ...
    fetch(`${process.env.REACT_APP_API_HOST}/updateaeropuerto/${this.state.id}`, {
      method: "POST",
      headers: {
  ...
  setData() {
    ...
    this.setState(
      ...
      () => {
        fetch(
          `${process.env.REACT_APP_API_HOST}/aeropuerto/${this.state.id}`,
          {
            method: "GET",
            headers: {
              Authorization: token
...
FormAeropuertos.contextType = Consumer;
