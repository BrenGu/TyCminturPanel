...
import FormAlquilerAutos from "./comalquileresauto/FormAlquilerAutos";
import Msg from "../utiles/Msg";

class alquilerautos extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: true,
  ...
  eliminarElemento(id) {
    fetch(`${process.env.REACT_APP_API_HOST}/alquilerauto/${id}`, {
      method: "DELETE",
      headers: {
  ...
  handleFromNovSubmit(event) {
    ...
    fetch(`${process.env.REACT_APP_API_HOST}/addalquilerauto`, {
      method: "POST",
      headers: {
        Authorization: "asdssffsdff",
        "Content-Type": "application/json"
  ...
  getData() {
    fetch(`${process.env.REACT_APP_API_HOST}/alquileresauto`, {
      method: "GET",
      headers: {
        Authorization: ""
      }
      ...
      .then(
        result => {
          if (!result.err) {
            this.setState({
              alquileres: result.data.registros
            });
          } else {
            this.setState({
              msg: {
  ...
  render() {
    const lista_guias = this.state.alquileres.map(novedad => {
      return (
        <FormAlquilerAutos
            ...
            >
              <div className="grid-noveades">
                ...
                <div>
                    <div className="col">
                        <div className="form-group">
                        <label htmlFor="codigo">Mail</label>
                        <input
                            type="text"
                            name="mail"
                            id="mail"
                ...
                <div className="col">
                  <div className="d-flex justify-content-between">
                    ...
                    <button type="submit" className="btn btn-primary">
                      <i className="fas fa-arrow-down" /> Agregar Alquiler
                    </button>
                  </div>
                </div>
              </div>
