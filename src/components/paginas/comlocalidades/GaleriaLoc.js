...
class GaleriaLoc extends Component {
    ...
    handleImgChange = (event) => {
        ...
        fetch(`${process.env.REACT_APP_API_HOST}/ciudad/${this.state.idLocalidad}/imagen`, {
            method: "POST",
            headers: {
    ...
    findGalery = () => {
        fetch(`${process.env.REACT_APP_API}/ciudad/${this.state.idLocalidad}/imagen`, {
            method: "GET",
            headers: {
                "Authorization": "asdssffsdff",
                //"Content-Type": "multipart/form-data"
            ...
            return(
                <div className="col-sm-6 col-md-4" key={`g-${g.id}`}>
                    <div className="card bg-dark text-white mb-2">
                        <img className="card-img" src={`${process.env.REACT_APP_API_HOST}/recursos/ciudadesFotos/${g.foto}`} alt="Img" />
                        <div className="card-img-overlay">
                            <div className="btn close bg-dark p-2 rounded" aria-label="Close" onClick={(e) => this.askDelete(g.id, g.foto, e)}>
                        ...
                        <div className="row">
                            <div className="col-sm-12 col-md-12">
                                <div className="d-flex justify-content-end mb-3">
                                    <input id={`uploadImage-${this.state.idLocalidad}`} name={`uploadImage-${this.state.idLocalidad}`} type="file" className="d-none" accept="image/*" onChange={this.handleImgChange} />
                                    <button className="btn btn-primary" onClick={(e) => {document.getElementById("uploadImage-" + this.state.idLocalidad).click()}}><i className="fas fa-camera"></i></button>
                                </div>
                            </div>
                        </div>
                        <div className="row">
